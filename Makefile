...
all: build/mrhat-battery-aitia_$(VERSION)-1_all.deb
	@true

build/mrhat-battery-aitia_$(VERSION)-1_all.deb : build/mrhat-battery-aitia.dtbo deb/DEBIAN/*
	mkdir -p build
	mkdir -p deb/boot/overlays/
	cp build/mrhat-battery-aitia.dtbo deb/boot/overlays/
	dpkg-deb -Zxz --root-owner-group --build deb build/mrhat-battery-aitia_$(VERSION)-1_all.deb
...
build/mrhat-battery-aitia.dtbo: build/mrhat-battery-simple.dts.pre
	mkdir -p build/
	dtc  -I dts -O dtb -o build/mrhat-battery-aitia.dtbo build/mrhat-battery-simple.dts.pre

deploy: all
	rsync -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null" -avhz --progress build/mrhat-battery-aitia_$(VERSION)-1_all.deb $(TARGET):/tmp/
...
quickdeploy: build/mrhat-battery-aitia.dtbo
	scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null build/mrhat-battery-aitia.dtbo $(TARGET):/tmp/
